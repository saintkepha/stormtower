      ...
      return async.whilst((function(_this) {
        ...
        return function(repeat) {
          ...
          try {
            ...
            req = http.request('/');
            req.target = 8000;
            return _this.bolt.relay(req, function(reply, body) {
              var checksum, err, md5, status;
              if (!(reply instanceof Error)) {
                try {
                  ...
                  if (checksum !== _this.checksum) {
                    ...
                    _this.status = status;
                    _this.emit('changed', status, checksum);
                    if (callback != null) {
                      callback(status);
                    }
                  }
                  ...
                  err = _error;
                  _this.log("unable to parse reply:", body);
                  _this.log("error:", err);
                }
              } else {
                _this.log("error:", reply);
              }
