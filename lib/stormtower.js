...
(function() {
    ...
    function StormTower(config) {
      ...
      this.clients.on('added', (function(_this) {
        return function(bolt) {
          ...
          return minion.once('ready', function() {
            _this.app.io.sockets.emit('minion.new', bolt.id, minion);
            _this.minions.add(bolt.id, minion);
            return minion.on('changed', function() {
              _this.minions.emit('changed');
              return _this.app.io.sockets.emit('minion.update', bolt.id, minion);
            });
          });
        };
      })(this));
      this.clients.on('removed', (function(_this) {
        return function(bolt) {
          ...
          entry = _this.minions.get(bolt.id);
          if (entry != null) {
            _this.app.io.sockets.emit('minion.remove', bolt.id, entry);
          }
  ...
  if (require.main === module) {
    ...
    process.on('uncaughtException', function(err) {
      return agent.log('ALERT.. caught exception', err, err.stack);
    });
  }
