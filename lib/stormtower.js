...
(function() {
  var StormBolt, StormTower, async, crypto, http,
  ...
  StormTower = (function(_super) {
    ...
    function StormTower(config) {
      ...
      this.boltServerHost = this.config.stormbolt.split(":")[0];
      this.boltServerPort = this.config.stormbolt.split(":")[1];
    ...
    StormTower.prototype.cnameDiscovery = function() {
      ...
      req.on("error", (function(_this) {
        return function(err) {
          return _this.log('[ERROR] [cnameDiscovery] error ', err);
        };
      })(this));
      return req.end();
    };
    ...
    StormTower.prototype.stormflashPolling = function() {
      ...
      return async.each(activatedEndpoints, ((function(_this) {
        return function(cname, callback) {
          var boltHeaders, req;
          _this.log("[stormflashPolling] " + cname + "] polling started");
          boltHeaders = {
          ...
          req = http.request(httpOptions, function(res) {
            ...
            return res.on("end", function() {
              ...
              _this.log("[stormflashPolling] " + cname + " response status ", res.statusCode);
              if (res.statusCode === 200) {
                ...
                _this.log("[stormflashPolling] " + cname + " stormflash removed from master table");
          ...
          req.on("error", function(err) {
            _this.log("[stormflashPolling] " + cname + " error ", err);
            _this.log("[stormflashPolling] " + cname + " stormflash removed from master table");
            delete allEndPoints.StormResponses[cname];
            callback(err + cname);
            return _this.next;
        ...
        return function(err) {
          var globalMD5, _ref1;
          if (err == null) {
            _this.log('[stormflashPolling] polling successful');
          } else {
            _this.log('[stormflashPolling] polling error ', err);
          }
          globalMD5 = crypto.createHash("md5");
          _ref1 = allEndPoints.StormResponses;
          for (key in _ref1) {
            value = _ref1[key];
            _this.log('[stormflashPolling] adding md5 checksum of ', key);
            globalMD5.update(JSON.stringify(allEndPoints.StormResponses[key].Response));
          }
          allEndPoints.globalChecksum = globalMD5.digest("hex");
          _this.log('[stormflashPolling] global md5 checksum ', allEndPoints.globalChecksum);
          _this.log('---------------- MASTER TABLE ----------------');
    ...
    httpReqSender = function(httpOptions, reqBody, timeout, cname, callback) {
      ...
      req = http.request(httpOptions, (function(_this) {
        return function(res) {
          ...
          return res.on("end", function() {
            var _ref;
            _this.log("[httpReqSender] " + cname + " response status ", res.statusCode);
            returnObj.status = res.statusCode;
            if (_ref = res.statusCode, __indexOf.call(successStatusCodes, _ref) >= 0) {
              _this.log("[httpReqSender] " + cname + " response: ", result);
              returnObj.output = JSON.parse(result);
            } else {
              if (result) {
                _this.log("[httpReqSender] " + cname + " failure: ", result.split('\n')[0]);
                returnObj.output = {
                  error: result.split('\n')[0]
                };
              } else {
      ...
      req.on("error", (function(_this) {
        return function(err) {
          _this.log("[httpReqSender] " + cname + " error ", err);
          returnObj.output = {
            error: err
          };
          return callback(returnObj);
      ...
      req.setTimeout(timeout, (function(_this) {
        return function(reply) {
          _this.log("[httpReqSender] " + cname + " timeout: ");
